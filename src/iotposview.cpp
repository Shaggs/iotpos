...
iotposView::iotposView() //: QWidget(parent)
{
 ...
 //   QTimer::singleShot(2000, timerCheckDb, SLOT(start()));
  ...
  dlgLogin = new LoginWindow(i18n("Welcome to IotPOS"),
...
void iotposView::showChangeDate()
{
  ui_mainview.groupSaleDate->show();
}

void iotposView::setupGridView()
{
...
void iotposView::changeDiscValidator()
{
...
void iotposView::verifyDiscountEntry()
{
...
void iotposView::timerTimeout()
{
  emit signalUpdateClock();
}

void::iotposView::clearLabelSearchMsg()
{
  ui_mainview.labelSearchMsg->clear();
}

void iotposView::setTheSplitterSizes(QList<int> s)
{
  ui_mainview.splitter->setSizes(s);
}

QList<int> iotposView::getTheSplitterSizes()
{
  return ui_mainview.splitter->sizes();
}

void iotposView::setTheGridSplitterSizes(QList<int> s)
{
  ui_mainview.splitterGrid->setSizes(s);
}

QList<int> iotposView::getTheGridSplitterSizes()
{
...
void iotposView::settingsChanged()
{
...
{
  //save new settings on db -to avoid double settings on iotpos and iotstock-
  Azahar *myDb = new Azahar;
  myDb->setDatabase(db);
...
void iotposView::settingsChangedOnInitConfig()
{
...
void iotposView::showSearchItemWidget()
{
...
void iotposView::askForIdToCancel()
{
...
void iotposView::askForTicketToReturnProduct()
{
  bool continuar=false;
  if (Settings::lowSecurityMode()) {//qDebug()<<"LOW security mode";
    continuar=true;
...
void iotposView::comboCurrencyOnChange()
{
    Azahar *myDb = new Azahar();
    myDb->setDatabase(db);
...
void iotposView::doCurrencyConversion()
{
...
void iotposView::acceptCurrencyConversion()
{
...
void iotposView::populateCardTypes()
{
    Azahar *myDb = new Azahar();
    myDb->setDatabase(db);
...
QString iotposView::getLoggedUser()
{
  return loggedUser;
}

QString iotposView::getLoggedUserName(QString id)
{
  QString uname = "";
  Azahar *myDb = new Azahar;
  myDb->setDatabase(db);
...
int iotposView::getUserRole(qulonglong id)
{
  int role = 0;
  Azahar *myDb = new Azahar;
  myDb->setDatabase(db);
...
qulonglong iotposView::getLoggedUserId(QString uname)
{
  unsigned int iD=0;
  Azahar *myDb = new Azahar;
  myDb->setDatabase(db);
...
bool iotposView::validAdminUser()
{
  bool result = false;
...
void iotposView::doSearchItemDesc()
{
  //clear last search
  ...
  for (int i = 0; i < pList.size(); ++i) {
     ...
     int rowCount = ui_mainview.tableSearch->rowCount();
      ...
      else if (pInfo.stockqty > 0 && pInfo.stockqty < Settings::stockAlertValue() ) {//NOTE:This must be shared between iotpos and iotstock
...
int iotposView::getItemRow(QString c)
{
  int result = 0;
...
void iotposView::refreshTotalLabel()
{
    buyPoints = 0;
...
RoundingInfo iotposView::roundUsStandard(const double &number)
{
    RoundingInfo result;
...
void iotposView::doEmitSignalQueryDb()
{
  emit signalQueryDb(ui_mainview.editItemCode->text());
}

bool iotposView::incrementTableItemQty(QString code, double q)
{
  double qty  = 1;
...
void iotposView::updateItem(ProductInfo prod)
{
...
int iotposView::doInsertItem(QString itemCode, QString itemDesc, double itemQty, double itemPrice, double itemDiscount, QString itemUnits)
{
...
void iotposView::itemDoubleClicked(QTableWidgetItem* item)
{
  int row = item->row();
  double dqty = 0.0;
  bool   ok   = false;
...
void iotposView::itemSearchDoubleClicked(QTableWidgetItem *item)
{
  int row = item->row();
...
void iotposView::displayItemInfo(QTableWidgetItem* item)
{
  int row = item->row();
...
QString iotposView::getCurrentTransactionString()
{
  return QString::number(currentTransaction);
}

qulonglong  iotposView::getCurrentTransaction()
{
  return currentTransaction;
}

void iotposView::createNewTransaction(TransactionType type)
{
  ...
  if ( operationStarted ) {
    ...
    if (currentTransaction <= 0) {
      KMessageBox::detailedError(this, i18n("IotPOS has encountered an error when openning database, click details to see the error details."), myDb->lastError(), i18n("Create New Transaction: Error"));
    }
    else {
      ...
      foreach(SpecialOrderInfo siInfo, specialOrders) {
        ...
        if (siInfo.status == stReady) {
          if ( siInfo.completePayment ) {
            ...
            //      ALSO the price and cost is for ONE PACK. If incrementing/decrementing at iotpos or at
...
void iotposView::freezeWidgets()
{
  emit signalDisableUI();
}

void iotposView::unfreezeWidgets()
{
  emit signalEnableUI();
  startAgain();
  ui_mainview.editItemCode->setFocus();
...
void iotposView::preCancelCurrentTransaction()
{
...
void iotposView::deleteCurrentTransaction()
{
  ...
  if (myDb->deleteTransaction(getCurrentTransaction())) {
    ...
    KMessageBox::detailedError(this, i18n("IotPOS has encountered an error when querying the database, click details to see the error details."), myDb->lastError(), i18n("Delete Transaction: Error"));
  }
  delete myDb;
}
...
void iotposView::cancelTransaction(qulonglong transactionNumber)
{
...
void iotposView::startOperation(const QString &adminUser)
{
...
void iotposView::_slotDoStartOperation()
{
  //simply call the other...
  slotDoStartOperation(true);
}

void iotposView::slotDoStartOperation(const bool &ask)
{
...
void iotposView::showBalance(QStringList lines)
{
  if (Settings::showDialogOnPrinting())
  {
...
void iotposView::printBalance(QStringList lines)
{
...
void iotposView::setupClientsModel()
{
    if (!db.isOpen()) {
        connectToDb();
    }
...
void iotposView::modifyClientsFilterModel()
{
...
void iotposView::modifyClientsFilterModelB()
{
...
void iotposView::populateCategoriesHash()
{
  Azahar * myDb = new Azahar;
  myDb->setDatabase(db);
...
void iotposView::listViewOnClick(const QModelIndex & index)
{
  //getting data from model...
  const QAbstractItemModel *model = index.model();
  int row = index.row();
...
void iotposView::updateModelView()
{
...
void iotposView::hideProductsGrid()
{
  ui_mainview.frameGridView->hide();
}

void iotposView::showPriceChecker()
{
...
void iotposView::updateClientInfo()
{
  QString dStr;
  BasketPriceCalculationService basketPriceCalculationService;
  BasketPriceSummary summary = basketPriceCalculationService.calculateBasketPrice(this->productsHash, this->clientInfo, oDiscountMoney);
...
void iotposView::setHistoryFilter() {
...
void iotposView::setupHistoryTicketsModel()
{
...
void iotposView::setupTicketView()
{
...
void iotposView::itemHIDoubleClicked(const QModelIndex &index){
  if (db.isOpen()) {
    //getting data from model...
    const QAbstractItemModel *model = index.model();
    int row = index.row();
...
void iotposView::printSelTicket()
{
...
void iotposView::printTicketFromTransaction(qulonglong transactionNumber)
{
  QList<TicketLineInfo> ticketLines;
  TicketInfo ticket;
  ticketLines.clear();
...
void iotposView::log(const qulonglong &uid, const QDate &date, const QTime &time, const QString &text)
{
  Azahar *myDb = new Azahar;
  myDb->setDatabase(db);
  myDb->insertLog(uid, date, time, "[ IOTPOS ] "+text);
  delete myDb;
}
...
void iotposView::specialOrderComplete()
{
...
void iotposView::updateBalance(bool finish)
{
  Azahar *myDb = new Azahar;
  myDb->setDatabase(db);
...
void iotposView::updateTransaction()
{
  //fill info
  TransactionInfo info;
...
void  iotposView::occasionalDiscount()
{
  bool continuar=false;
  oDiscountMoney = 0;
  if (Settings::lowSecurityMode()) { //qDebug()<<"LOW security mode";
...
void iotposView::applyOccasionalDiscount()
{
...
//NOTE: Here the store owner/admin needs to know that when iotpos makes a product reservation the product qty in stock is decremented.
...
void iotposView::suspendReservation()
{
...
void iotposView::resumeReservation()
{
    qDebug()<<"finishingReservation:"<<finishingReservation<<" startingReservation:"<<startingReservation;
...
void iotposView::postponeReservation()
{
...
void iotposView::addReservationPayment()
{
...
void iotposView::insertCashInForReservationPayment(const qulonglong &rid, const double &amount)
{
    Azahar *myDb = new Azahar;
    myDb->setDatabase(db);
...
void iotposView::filterClientForCredit()
{
...
void iotposView::tenderedChanged()
{
    if (crInfo.id <= 0 || crClientInfo.id <= 0) return; //PLEASE ENTER CLIENT CODE FIRST!!!
...
void iotposView::printCreditReport()
{
    if (Settings::printTicket()) {
...
void iotposView::insertCashInForCredit(const CreditInfo &credit, const double &amount)
{
    Azahar *myDb = new Azahar;
    myDb->setDatabase(db);
...
void iotposView::calculateTotalForClient()
{
    ui_mainview.creditContent->clear();
