...
iotstock::iotstock()
    //: KXmlGuiWindow(0,Qt::FramelessWindowHint ),
     : KXmlGuiWindow( ),
      ...
      m_printer(0)
{
    setObjectName(QLatin1String("iotstock"));
    // accept dnd
...
void iotstock::setConnected()
{
  //a workaround.. just to dont modify the code in iotstockview
  setConnection(true);
}

void iotstock::setDisconnected()
{
  //a workaround.. just to dont modify the code in iotstockview
  setConnection(false);
}

void iotstock::setConnection(bool yes)
{
  if ( yes ) {
    led->on();
    enableUI();
...
**/
void iotstock::showDBConfigDialog()
{
...
void iotstock::optionsPreferences()
{
    //avoid to have 2 dialogs shown
    if ( KConfigDialog::showDialog( "settings" ) )  {
        return;
    ...
    dialog->addPage(printerSettingsDlg, i18n("Printers"), "iotpos-printer");
...
void iotstock::changeStatusbar(const QString& text)
{
    // display the text on the statusbar
    statusBar()->showMessage(text);
}

void iotstock::changeCaption(const QString& text)
{
    // display the text on the caption
    setCaption(text);
}

bool iotstock::queryClose()
{
  m_view->closeDB();
  return true;
}
