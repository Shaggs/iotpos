...
**/
void iotpos::showDBConfigDialog()
{
...
void iotpos::optionsPreferences()
{
    //avoid to have 2 dialogs shown
    if ( KConfigDialog::showDialog( "settings" ) )  {
        return;
   ...
   //NOTE: What icon to use for this??
    ...
    dialog->addPage(printersSettingDlg, i18n("Printer"), "iotpos-printer");
...
void iotpos::disableLogin()
{
  QAction *action = actionCollection()->action("login");
  action->setDisabled(true);
}

void iotpos::enableLogin()
{
  QAction *action = actionCollection()->action("login");
  action->setEnabled(true);
}

void iotpos::enableUi()
{
...
void iotpos::sLitemActivated(QListWidgetItem * item)
{
  ui_pref_style.kcfg_styleName->setText(item->text());
}


void iotpos::updateClock()
{
...
void iotpos::changeStatusbar(const QString& text)
{
    // display the text on the statusbar
  statusBar()->showMessage(text, 1500);
}

void iotpos::changeCaption(const QString& text)
{
    // display the text on the caption
  setCaption(text);
}

bool iotpos::queryClose()
{
